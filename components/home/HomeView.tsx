...
const QuickActionCard: React.FC<{
  ...
  description: string;
  darkMode?: boolean;
}> = ({ title, to, icon, description, darkMode }) => (
  <Link
    to={to}
...
export const HomeView: React.FC<HomeViewProps> = ({
  ...
  const plannerSummary: PlannerSummary = {
    upcomingPosts: scheduledPosts.filter(
      (p) => new Date(p.scheduledTime) >= new Date() && p.status === "scheduled"
    ).length,
    pastDuePosts: scheduledPosts.filter(
      (p) => new Date(p.scheduledTime) < new Date() && p.status === "scheduled"
    ).length,
  };
  ...
  const trackerSummary: TrackerSummary = {
    ...
    totalTrackedPosts: trackedAccounts.reduce(
  ...
  const fetchTrends = async () => {
    ...
    if (trends) {
      setTrendingTopics(
        trends.map(
          (t) => `${t.trendName}: ${t.description.substring(0, 100)}...`
        )
      );
    } else {
  ...
  const summaryCardClass = darkMode
    ...
    <div className={`p-1 ${darkMode ? "text-slate-100" : "text-gray-800"}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
        <div className={`${summaryCardClass} p-5 rounded-lg shadow-lg`}>
          ...
          <p className={labelClass}>
            Upcoming Posts:{" "}
            <span className={valueClass}>{plannerSummary.upcomingPosts}</span>
          </p>
          {plannerSummary.pastDuePosts > 0 && (
            <p className={`${labelClass} text-yellow-500 dark:text-yellow-400`}>
              Past Due:{" "}
              <span
                className={`${valueClass} text-yellow-500 dark:text-yellow-400`}
              >
                {plannerSummary.pastDuePosts}
              </span>
            </p>
          )}
          <Link
        ...
        <div className={`${summaryCardClass} p-5 rounded-lg shadow-lg`}>
          ...
          <p className={labelClass}>
            Tracked Accounts:{" "}
            <span className={valueClass}>{trackerSummary.trackedAccounts}</span>
          </p>
          <p className={labelClass}>
            Total Tracked Posts:{" "}
            <span className={valueClass}>
              {trackerSummary.totalTrackedPosts}
            </span>
          </p>
          <Link
            to="/tracker"
        ...
        <div className={`${summaryCardClass} p-5 rounded-lg shadow-lg`}>
          ...
          <p className={labelClass}>
            ...
            <span
              className={`${valueClass} ${
                monetizationSummary.netProfit >= 0
                  ? darkMode
                    ? "text-sky-400"
                    : "text-sky-600"
                  : darkMode
          ...
          <p className={labelClass}>
            Total Earnings:{" "}
            <span
              className={`${valueClass} text-base ${
                darkMode ? "text-green-400" : "text-green-500"
              }`}
            >
              ${monetizationSummary.totalEarnings.toFixed(2)}
            </span>
          </p>
          <p className={labelClass}>
            Total Expenses:{" "}
            ...
            >
              ${monetizationSummary.totalExpenses.toFixed(2)}
            </span>
          </p>
          <Link
            to="/monetization"
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <QuickActionCard
            ...
            description="Go directly to the content planner to schedule new posts."
            darkMode={darkMode}
          />
          <QuickActionCard
            ...
            description="Start tracking performance for a new social media account or specific posts."
            darkMode={darkMode}
          />
          <QuickActionCard
            ...
            description="Log your latest income or business expenses to keep finances up to date."
            darkMode={darkMode}
          />
        </div>
      </div>
      ...
      {isGeminiAvailable() && (
        ...
        >
          ...
          <div className="mb-4">
            ...
            <div className="flex space-x-2">
              <input
                ...
                className={`${
                  darkMode
                    ? "bg-slate-700 border-slate-600 focus:ring-teal-500 focus:border-teal-500"
                    : "bg-white border-gray-300 focus:ring-primary-500 focus:border-primary-500"
                } flex-grow sm:text-sm border rounded-md p-2 shadow-sm`}
              />
              <Tooltip
                text="Get current content trends for your niche using AI."
                darkMode={darkMode}
              >
                <button
                  onClick={fetchTrends}
                  ...
                  className={`px-4 py-2 text-sm font-medium rounded-md flex items-center justify-center ${
                    darkMode
                      ? "bg-teal-500 hover:bg-teal-600 disabled:bg-slate-600"
          ...
          {trendsError && (
            <p className="text-sm text-red-500 mt-1">{trendsError}</p>
          )}
          {trendingTopics.length > 0 && (
            <div>
              ...
              >
                Trending Topics Suggested by AI:
              </h3>
              <ul
      ...
      >
        ...
        >
          <li className="flex items-start">
            ...
            <span>
              Use the{" "}
              <strong className={darkMode ? "text-sky-300" : "text-sky-700"}>
                Planner
              </strong>{" "}
